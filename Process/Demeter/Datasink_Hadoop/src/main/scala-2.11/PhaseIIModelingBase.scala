...
/**
  ...
  * The pipleine for training base models
  */
object PhaseIIModelingBase extends {
  ...
  def main(args: Array[String]) {
    ...
    for (itr <- 0 to 9) {
      ...
      utility.appendParquetFile(sparkSession, df, dir + "/" + "ResultCache/" + outerIteration + modelName + iteration)

    }
    val df1 = utility.readParquetFile(sparkSession, dir + "/" + "ResultCache/" + outerIteration + modelName + iteration)
    ...
    utility.appendParquetFile(sparkSession, df3, dir + "/" + "Result/" + outerIteration + modelName + iteration)
    sparkSession.stop()
